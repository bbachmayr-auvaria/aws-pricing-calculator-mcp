...
def save_estimate(estimate_dict):
    ...
    with open(debug_file, "a") as f:
        f.write(f"\n{'='*60}\n")
        f.write(
            f"[DEBUG save_estimate] Called at {datetime.now(timezone.utc).isoformat()}\n"
        )
        f.write(f"[DEBUG save_estimate] Estimate type: {type(estimate_dict)}\n")
        f.write(
            f"[DEBUG save_estimate] Estimate keys: {list(estimate_dict.keys()) if isinstance(estimate_dict, dict) else 'N/A'}\n"
        )
        ...
        if not isinstance(estimate_dict, dict):
            ...
            raise TypeError(
                f"estimate_dict must be a dict, got {type(estimate_dict).__name__}"
            )
...
def main():
    ...
    parser.add_argument(
        "--local-only", action="store_true", help="Only validate, don't upload"
    )
    ...
    total_services = sum(
        len(g.get("services", {})) for g in estimate.get("groups", {}).values()
    )
